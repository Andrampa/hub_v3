...
- The dataset map's background is the FAO-customised ArcGIS vector style (item `58420c8cfe754b11ba4d9ecd07a62da0`), drawn by MapLibre inside Leaflet's tile pane beneath the DIEM features (`src/components/datasetBasemap.ts`). It shows UN boundaries, including Abyei, and UN country names: the Hub switches on the style's hidden UN label layers and gives each vector source explicit tiles (`src/lib/unBasemapStyle.ts`); nothing else in the authored style changes. MapLibre loads with the map, not the page. The map swaps to the Hub's own UN Geodata simplified geometry, once, when the style or its service metadata cannot be read, when WebGL is unavailable, or afterwards when MapLibre reports that the UN boundaries cannot be drawn: three tile failures on a UN boundary source before any tile arrives, no boundary tile within 20 seconds, or a lost WebGL context (`src/components/basemapHealth.ts`). Isolated tile errors after boundaries loaded, and sprite or font errors, do not trigger it. The map shows the full disclaimer below it, because it can be panned to any disputed area. Neither case blocks filters, table previews, API links or downloads.
