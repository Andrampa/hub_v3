...
| Packaged dataset export | `GET https://hub.arcgis.com/api/download/v1/items/{itemId}/{format}` with `layers` and `where` | Active community identity | Excel, Shapefile, KML/KMZ, File Geodatabase, GeoPackage and SQLite downloads |
| Legacy Hub slug lookup | `GET https://hub.arcgis.com/api/v3/datasets?filter[slug]={org::title}` | None | Translating an address the previous site served into a catalogue product |
| Abyei area and limits (build time only) | `GET https://geoservices.un.org/arcgis/rest/services/ClearMap_Topo/MapServer/{110,94}/query` (UN Geospatial ClearMap) | Public | `npm run build:boundaries` adds Abyei, absent from UN Geodata simplified |
