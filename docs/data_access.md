...
- Microdata (workspace tab): FAM as the default route with its publication lag, direct request as the exception, temporary grants, household collections for household-data members, and the full microdata licence from `src/components/MicrodataLicence.tsx`. The survey-first picker merges accessible master and temporary-grant scopes, allows up to ten surveys per package, offers V3 mandatory/optional choice and a coded / labelled / both values choice, and performs live access-and-record-count preflight before the cancellable archive download. Labels come from layer coded-value domains and are offered only while the live domains match the digests recorded by `scripts/audit_microdata_domains.py` in `src/data/auditedDomains.json`; each survey folder carries `value_labels.csv` for verified tables. See `docs/microdata_access_restructure.md` §8a. The browser package limits are 50,000 records, 40 MB of actual encoded CSV (coded and labelled together) and 20 source tables; if a limit or live authorization check fails, no partial archive is offered.
...
- CSV formula-injection rule (all CSV exports: explorer CSV, aggregated survey packages including `surveys.csv`, household microdata packages). `csvCell` in `src/services/dataExplorer.ts` prefixes a single quote (`'`) to any text value that begins with `=`, `+`, `-`, `@`, tab or carriage return, following OWASP CSV-injection guidance, because respondent free text can otherwise run as a spreadsheet formula. Values that are a whole plain decimal number (`-99`, `+5`, `-3.5e2`) are never altered. Header names use the same rule. Quoting then applies to `"`, `,`, CR and LF. Because this changes the bytes of affected text, packages state the rule (`CSV_TEXT_NEUTRALISATION`) in README.txt and `manifest.json` (`csv_text_neutralisation`); analysts reading with R or pandas should strip the leading `'` from those values. The Excel export needs no equivalent: `write-excel-file` stores text as string cells, never formulas, so a prefix would only add a visible quote. The generated Python/R extraction scripts write their own CSVs and do not apply this rule.
