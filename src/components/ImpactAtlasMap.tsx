...
export function ImpactAtlasMap({
  ...
  const { paths, path } = useMemo(() => {
    ...
    return {
      path,
      paths: worldAreas.map((area) => ({
        iso3: area.iso3,
        kind: area.kind,
        d: path(area.feature) || '',
        centroid: path.centroid(area.feature),
      })),
    }
  }, [visibleIso])
  ...
  const symbols = useMemo(() => {
    const centroidByIso = new Map(paths.filter(({ kind }) => kind === 'country').map(({ iso3, centroid }) => [iso3, centroid]))
  ...
  return (
    <>
      <div className="impact-map-wrap">
        ...
        {paths.map(({ iso3, kind, d }) => {
          if (kind === 'neutral') return <path className="impact-map-country map-area--neutral" d={d} key={iso3} />
          const summary = kind === 'country' ? countryByIso.get(iso3) : undefined
