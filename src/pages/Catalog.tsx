...
import { CatalogContentCard } from '../components/CatalogContentCard'
import { SiteFooter } from '../components/SiteFooter'
import { SiteHeader } from '../components/SiteHeader'
import { useCatalog } from '../hooks/useCatalog'
import { itemTheme, itemYear } from '../lib/catalog'
import { familySearchText, groupProductFamilies } from '../lib/productFamilies'
import { CONTENT_GROUP_ID } from '../services/arcgis'
import { countryDefinition, itemCountryCodes } from '../services/countries'
import type { ArcGISItem, SortOption } from '../types'
...
export default function Catalog() {
  ...
  const theme = params.get('theme') || 'All themes'
  const year = params.get('year') || 'All years'
  const requestedSort = params.get('sort') || 'newest'
  const sort: SortOption = requestedSort === 'oldest' || requestedSort === 'title' ? requestedSort : 'newest'
  const page = Math.max(1, Number(params.get('page')) || 1)

  const families = useMemo(() => groupProductFamilies(catalog?.items || []), [catalog])
  const themes = useMemo(() => [...new Set(catalog?.items.map(itemTheme) || [])].sort(), [catalog])
  const countries = useMemo(() => [...new Set(catalog?.items.flatMap(itemCountryCodes) || [])]
  ...
  const filteredFamilies = useMemo(() => {
    ...
    return families.filter((family) => (
      ...
      (country === 'All countries' || family.variants.some((item) => itemCountryCodes(item).includes(country))) &&
  ...
  }, [category, country, families, query, sort, theme, year])
  ...
  const hasFilters = Boolean(query || category !== 'All content' || country !== 'All countries' || theme !== 'All themes' || year !== 'All years' || sort !== 'newest')
      ...
      <main className="catalog-page" id="top">
        ...
        <section className="catalog-section" aria-labelledby="catalog-title">
          <div className="section-wrap">
            <div className="section-heading catalog-heading"><div><span className="kicker">Find evidence</span><h2 id="catalog-title">The complete collection</h2></div><p>Use country and theme when you know the context, or search across titles, descriptions and tags.</p></div>
            <div className="filter-bar catalog-filter-bar">
              ...
              <label><span>Content</span><select value={category} onChange={(event) => update('content', event.target.value, 'All content')}><option>All content</option>{Object.keys(typeGroups).map((value) => <option key={value}>{value}</option>)}</select></label>
              ...
              <label><span>Theme</span><select value={theme} onChange={(event) => update('theme', event.target.value, 'All themes')}><option>All themes</option>{themes.map((value) => <option key={value}>{value}</option>)}</select></label>
            ...
            {error ? <div className="error-state" role="alert"><strong>The public catalog could not be reached.</strong><p>{error}. Check your connection and try again.</p><button type="button" onClick={retry}>Retry</button></div> : !catalog ? <div className="loading-state" role="status"><span className="loader" /><strong>Connecting to the public DIEM catalog</strong><p>Gathering the latest resources from the DIEM content platform…</p></div> : <>
              <div className="results-meta" aria-live="polite"><p><strong>{filteredFamilies.length.toLocaleString()}</strong> products found</p><div>{hasFilters && <button type="button" className="clear-filters" onClick={clearFilters}>Clear filters</button>}<a href={`https://hqfao.maps.arcgis.com/home/group.html?id=${CONTENT_GROUP_ID}`} target="_blank" rel="noreferrer">View source group <span aria-hidden="true">↗</span></a></div></div>
